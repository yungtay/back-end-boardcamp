...
app.get("/categories", async (req, res) => {
    if (req.query.offset) {
      req.query.offset = stripHtml(req.query.offset).result.trim();
    }
    if(req.query.limit) {
        req.query.limit = stripHtml(req.query.limit).result.trim();
    }
    ...
    if(offset) {
        reqParms ++;
        query += `OFFSET $${reqParms} ROWS `
        if(preparedQuery){
            preparedQuery = [...preparedQuery, offset]
        } else {
    ...
    if(limit) {
        reqParms ++;
        query += `LIMIT $${reqParms} `
        if(preparedQuery){
           preparedQuery = [...preparedQuery, limit]
        } else {
            preparedQuery = [limit]
        }
    }
  try {
    const result = await connection.query(`SELECT * FROM categories ${query}`, preparedQuery);
...
app.get("/games", async (req, res) => {
  ...
  if(name) {
    reqParms ++;
    query += `WHERE games.name ILIKE $${reqParms} `
    if(preparedQuery){
        preparedQuery = [...preparedQuery, name + '%']
    } else {
        preparedQuery = [name + '%']
    }
}
...
app.get("/customers", async (req, res) => {
  ...
  if(cpf) {
    reqParms ++;
    query += `WHERE cpf ILIKE $${reqParms} `
    if(preparedQuery){
        preparedQuery = [...preparedQuery, cpf]
    } else {
        preparedQuery = [cpf]
    }
}
  ...
  try {
    ...
    await connection.query(
      ...
      [
        ...
        resultGameId.rows[0].pricePerDay * daysRented,
        null,
        null,
      ]
    );
...
app.get("/rentals", async (req, res) => {
  ...
  if(customerId) {
    reqParms ++;
    query += `WHERE "customerId" = $${reqParms} `
    if(preparedQuery){
        preparedQuery = [...preparedQuery, customerId]
    } else {
        preparedQuery = [customerId]
    }
}
if(gameId) {
    reqParms ++;
    query ? query += `AND "gameId" = $${reqParms} ` : query += `WHERE "gameId" = $${reqParms} `
    if(preparedQuery){
        preparedQuery = [...preparedQuery, gameId]
    } else {
        preparedQuery = [gameId]
    }
}
  if(offset) {
      reqParms ++;
...
//     ? ' WHERE "gameId" = $1'
//     : "";
//   const preparedQuery = customerId
//     ? [`${customerId}`]
//     : gameId
//     ? [`${gameId}`]
//     : "";
console.log(query, preparedQuery)
  try {
    const result = await connection.query(
      ...
      jsonb_build_object('id', games.id, 'name', games.name, 'categoryId', games."categoryId") AS game 
      FROM rentals 
...
app.post("/rentals/:id/return", async (req, res) => {
  ...
  try {
    ...
    let fee =
      dayjs("2021-06-27").diff(
        dayjs(resultRent.rows[0].rentDate).add(
          resultRent.rows[0].daysRented,
          "day"
        ),
        "day"
      ) * resultGamePrice.rows[0].pricePerDay;
    if (fee < 0) fee = 0;
    await connection.query(
      'UPDATE rentals SET "returnDate" = $1, "delayFee" = $2 WHERE id = $3',
      [dayjs("2021-06-27"), fee, id]
    );
    await connection.query(
      'UPDATE games SET "stockTotal" = "stockTotal" + 1 WHERE id = $1',
      [resultRent.rows[0].gameId]
